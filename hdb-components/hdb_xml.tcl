...
close $xml_fd
 ...
 if {$wellFormed ne ""} {
	...
	autopilot_sequence { set myvariable apsequence }
        mssqls_linux_server { if {![string match windows $::tcl_platform(platform)]} {
	set myvariable mssqls_server
		} 
	   }
        mssqls_linux_authent { if {![string match windows $::tcl_platform(platform)]} {
	set myvariable mssqls_authentication
		} 
	   }
        mssqls_linux_odbc { if {![string match windows $::tcl_platform(platform)]} {
	set myvariable mssqls_odbc_driver
		} 
	   }
	}
    } else {

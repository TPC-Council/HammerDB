...
if {[ tk_messageBox -title "Create Schema" -icon question -message "Ready to create a Scale Factor $db2_scale_fact HDB TPC-H schema under user [ string toupper $db2_tpch_user ]\n in database [ string toupper $db2_tpch_dbase ]?" -type yesno ] == yes} {
...
set ntimes 1
ed_edit_clear
set _ED(packagekeyname) "Db2 HDB TPC-H creation"
...
set _ED(packagekeyname) "Db2 HDB TPC-H"
.ed_mainFrame.mainwin.textFrame.left.text fastinsert end "#!/usr/local/bin/tclsh8.6
#EDITABLE OPTIONS##################################################

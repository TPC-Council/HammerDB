proc build_vsqltpch {} {
    ...
    if {[ tk_messageBox -title "Create Schema" -icon question -message "Ready to create a Scale Factor $vsql_scale_fact TPROC-H schema\n in host [string toupper $vsql_connector] under user [ string toupper $vsql_tpch_user ] in database [ string toupper $vsql_tpch_dbase ] with storage engine [ string toupper $vsql_tpch_storage_engine ]?" -type yesno ] == yes} {
        if { $vsql_num_tpch_threads eq 1 } {
            ...
            set maxvuser [ expr $vsql_num_tpch_threads + 1 ]
        }
        set suppo 1
        set ntimes 1
        ed_edit_clear
        set _ED(packagekeyname) "VillageSQL TPROC-H creation"
...
proc GatherStatistics { vsql_handler is_oceanbase partition_num} {
...
proc ConnectToVillageSQL { host port socket ssl_options user password is_oceanbase ob_tenant_name} {
    global mysqlstatus
    #ssl_options is variable length so build a connectstring
...
proc CreateTables { vsql_handler vsql_tpch_storage_engine } {
...
ENGINE = $vsql_tpch_storage_engine"
...
proc CreateOBTables { vsql_handler partitions_num } {
...
proc UpdateHeatwaveSchema { vsql_handler } {
    puts "VillageSQL UpdateHeatwaveSchema"
...
"
    for { set i 1 } { $i <= 5 } { incr i } {
        ...
        foreach query $separate {
          ...
          mysql::exec $vsql_handler $query
        }
    }
    return
}

proc mk_region { vsql_handler } {
    for { set i 1 } { $i <= 5 } {incr i} {
        set code [ expr {$i - 1} ]
        set text [ lindex [ lindex [ get_dists regions ] [ expr {$i - 1} ] ] 0 ]
        set comment [ TEXT_1 72 ]
        mysql::exec $vsql_handler "INSERT INTO REGION (`R_REGIONKEY`,`R_NAME`,`R_COMMENT`) VALUES ('$code' , '$text' , '$comment')"
    }
    mysql::commit $vsql_handler
}

proc mk_nation { vsql_handler } {
    for { set i 1 } { $i <= 25 } {incr i} {
        ...
        set comment [ TEXT_1 72 ]
        mysql::exec $vsql_handler "INSERT INTO NATION (`N_NATIONKEY`, `N_NAME`, `N_REGIONKEY`, `N_COMMENT`) VALUES ('$code' , '$text' , '$join' , '$comment')"
    }
    mysql::commit $vsql_handler
}

proc mk_supp { vsql_handler start_rows end_rows } {
    ...
    for { set i $start_rows } { $i <= $end_rows } { incr i } {
        ...
        if { ![ expr {$i % 1000} ] || $i eq $end_rows } {
            mysql::exec $vsql_handler "INSERT INTO SUPPLIER (`S_SUPPKEY`, `S_NATIONKEY`, `S_COMMENT`, `S_NAME`, `S_ADDRESS`, `S_PHONE`, `S_ACCTBAL`) VALUES $supp_val_list"
            mysql::commit $vsql_handler
            unset supp_val_list
        } else {
...
proc mk_cust { vsql_handler start_rows end_rows } {
    for { set i $start_rows } { $i <= $end_rows } { incr i } {
        set custkey $i
        ...
        if { ![ expr {$i % 1000} ] || $i eq $end_rows } {
            mysql::exec $vsql_handler "INSERT INTO CUSTOMER (`C_CUSTKEY`, `C_MKTSEGMENT`, `C_NATIONKEY`, `C_NAME`, `C_ADDRESS`, `C_PHONE`, `C_ACCTBAL`, `C_COMMENT`) values $cust_val_list"
            mysql::commit $vsql_handler
            unset cust_val_list
        } else {
...
proc mk_part { vsql_handler start_rows end_rows scale_factor } {
    for { set i $start_rows } { $i <= $end_rows } { incr i } {
        set partkey $i
        ...
        # end of psupp loop
        if { ![ expr {$i % 1000} ]  || $i eq $end_rows } {
            mysql::exec $vsql_handler "INSERT INTO PART (`P_PARTKEY`, `P_TYPE`, `P_SIZE`, `P_BRAND`, `P_NAME`, `P_CONTAINER`, `P_MFGR`, `P_RETAILPRICE`, `P_COMMENT`) VALUES $part_val_list"
            mysql::exec $vsql_handler "INSERT INTO PARTSUPP (`PS_PARTKEY`, `PS_SUPPKEY`, `PS_SUPPLYCOST`, `PS_AVAILQTY`, `PS_COMMENT`) VALUES $psupp_val_list"
            mysql::commit $vsql_handler
            unset part_val_list
...
proc mk_order { vsql_handler start_rows end_rows upd_num scale_factor oceanbase_db} {
    set refresh 100
    set delta 1
	...
	set totalprice [ expr double($totalprice) / 100 ]
        ...
        if { ![ expr {$i % 1000} ]  || $i eq $end_rows } {
            mysql::exec $vsql_handler "INSERT INTO LINEITEM (`L_SHIPDATE`, `L_ORDERKEY`, `L_DISCOUNT`, `L_EXTENDEDPRICE`, `L_SUPPKEY`, `L_QUANTITY`, `L_RETURNFLAG`, `L_PARTKEY`, `L_LINESTATUS`, `L_TAX`, `L_COMMITDATE`, `L_RECEIPTDATE`, `L_SHIPMODE`, `L_LINENUMBER`, `L_SHIPINSTRUCT`, `L_COMMENT`) VALUES $lineit_val_list"
            mysql::exec $vsql_handler "INSERT INTO ORDERS (`O_ORDERDATE`, `O_ORDERKEY`, `O_CUSTKEY`, `O_ORDERPRIORITY`, `O_SHIPPRIORITY`, `O_CLERK`, `O_ORDERSTATUS`, `O_TOTALPRICE`, `O_COMMENT`) VALUES $order_val_list"
            mysql::commit $vsql_handler
...
proc do_tpch { host port socket ssl_options scale_fact user password db vsql_tpch_storage_engine num_vu oceanbase_db ob_partition_num ob_tenant_name} {
    global mysqlstatus
    ...
    if { $threaded eq "SINGLE-THREADED" ||  $threaded eq "MULTI-THREADED" && $myposition eq 1 } {
        ...
        set vsql_handler [ ConnectToVillageSQL $host $port $socket $ssl_options $user $password $oceanbase_db $ob_tenant_name]
        ...
        if { $oceanbase_db == "true" } {
            CreateOBTables $vsql_handler $ob_partition_num
        } else {
            if { [string equal -nocase $vsql_tpch_storage_engine "Heatwave" ] } {
                CreateTables $vsql_handler "InnoDB"
            } else {
                CreateTables $vsql_handler $vsql_tpch_storage_engine
            }
        }
        if { $threaded eq "MULTI-THREADED" } {
    ...
    if { $threaded eq "SINGLE-THREADED" ||  $threaded eq "MULTI-THREADED" && $myposition != 1 } {
        ...
        mk_supp $vsql_handler [ lindex $sf_chunk 0 ] [ lindex $sf_chunk 1 ]
        puts "Loading CUSTOMER..."
        mk_cust $vsql_handler [ lindex $cust_chunk 0 ] [ lindex $cust_chunk 1 ]
        puts "Loading PART and PARTSUPP..."
        mk_part $vsql_handler [ lindex $part_chunk 0 ] [ lindex $part_chunk 1 ] $scale_fact
        puts "Loading ORDERS and LINEITEM..."
        mk_order $vsql_handler [ lindex $ord_chunk 0 ] [ lindex $ord_chunk 1 ] [ expr {$upd_num % 10000} ] $scale_fact $oceanbase_db
	...
	UpdateHeatwaveSchema $vsql_handler
        puts "Migrating data to Heatwave COMPLETE"
    }
    if { $threaded eq "SINGLE-THREADED" || $threaded eq "MULTI-THREADED" && $myposition eq 1 } {
        GatherStatistics $vsql_handler $oceanbase_db $ob_partition_num
        puts "[ string toupper $db ] SCHEMA COMPLETE"
        return
    }
}
}
        .ed_mainFrame.mainwin.textFrame.left.text fastinsert end "do_tpch $vsql_host $vsql_port $vsql_socket {$vsql_ssl_options} $vsql_scale_fact $vsql_tpch_user [ quotemeta $vsql_tpch_pass ] $vsql_tpch_dbase $vsql_tpch_storage_engine $vsql_num_tpch_threads $vsql_tpch_obcompat $vsql_ob_partition_num $vsql_ob_tenant_name"
    } else { return }
}
...
set total_querysets $vsql_total_querysets ;# Number of query sets before logging off
set RAISEERROR \"$vsql_raise_query_error\" ;# Exit script on VillageSQL query error (true or false)
set VERBOSE \"$vsql_verbose\" ;# Show query text and output
set scale_factor $vsql_scale_fact ;#Scale factor of the tpc-h schema
...
set user \"$vsql_tpch_user\" ;# VillageSQL user
set password \"[ quotemeta $vsql_tpch_pass ]\" ;# Password for the VillageSQL user
set db \"$vsql_tpch_dbase\" ;# Database containing the TPC Schema
set refresh_on \"$vsql_refresh_on\" ;#First User does refresh function
set update_sets $vsql_update_sets ;#Number of sets of refresh function to complete
set trickle_refresh $vsql_trickle_refresh ;#time delay (ms) to trickle refresh function
set REFRESH_VERBOSE \"$vsql_refresh_verbose\" ;#report refresh function activity
set tpch_obcompat \"$vsql_tpch_obcompat\" ;# Oceanbase compatible
set ob_tenant_name \"$vsql_ob_tenant_name\" ;# Oceanbase tenant name
#OPTIONS
"
...
proc standsql { vsql_handler sql RAISEERROR } {
    global mysqlstatus
    set oput ""
    catch { set oput [ join [ mysql::sel $vsql_handler "$sql" -list ] ] }
    if { $mysqlstatus(code)  } {
        if { $RAISEERROR } {
...
proc ConnectToVillageSQL { host port socket ssl_options user password db is_oceanbase ob_tenant_name} {
    global mysqlstatus
    #ssl_options is variable length so build a connectstring
    if { ($is_oceanbase == "false" ) && ([ chk_socket $host $socket ] eq "TRUE") } {
        set use_socket "true"
...
proc mk_order_ref { vsql_handler upd_num scale_factor trickle_refresh REFRESH_VERBOSE } {
    ...
    for { set i $startindex } { $i <= $endindex } { incr i } {
        ...
        for { set l 0 } { $l < $lcnt } {incr l} {
            ...
            mysql::exec $vsql_handler "INSERT INTO LINEITEM (`L_SHIPDATE`, `L_ORDERKEY`, `L_DISCOUNT`, `L_EXTENDEDPRICE`, `L_SUPPKEY`, `L_QUANTITY`, `L_RETURNFLAG`, `L_PARTKEY`, `L_LINESTATUS`, `L_TAX`, `L_COMMITDATE`, `L_RECEIPTDATE`, `L_SHIPMODE`, `L_LINENUMBER`, `L_SHIPINSTRUCT`, `L_COMMENT`) VALUES (str_to_date('$lsdate','%Y-%M-%d'),'$lokey', '$ldiscount', '$leprice', '$lsuppkey', '$lquantity', '$lrflag', '$lpartkey', '$lstatus', '$ltax', str_to_date('$lcdate','%Y-%M-%d'), str_to_date('$lrdate','%Y-%M-%d'), '$lsmode', '$llcnt', '$linstruct', '$lcomment')"
        }
	set totalprice [ expr double($totalprice) / 100 ]
        if { $REFRESH_VERBOSE } {
            puts "Refresh Insert Orderkey $okey..."
        }
        mysql::exec $vsql_handler "INSERT INTO ORDERS (`O_ORDERDATE`, `O_ORDERKEY`, `O_CUSTKEY`, `O_ORDERPRIORITY`, `O_SHIPPRIORITY`, `O_CLERK`, `O_ORDERSTATUS`, `O_TOTALPRICE`, `O_COMMENT`) VALUES (str_to_date('$date','%Y-%M-%d'), '$okey', '$custkey', '$opriority', '$spriority', '$clerk', '$orderstatus', '$totalprice', '$comment')"
        if { ![ expr {$i % 1000} ] } {
            mysql::commit $vsql_handler
        }
    }
...
proc del_order_ref { vsql_handler upd_num scale_factor trickle_refresh REFRESH_VERBOSE } {
    ...
    for { set i $startindex } { $i <= $endindex } { incr i } {
        ...
        mysql::exec $vsql_handler "DELETE FROM LINEITEM WHERE L_ORDERKEY = $okey"
        mysql::exec $vsql_handler "DELETE FROM ORDERS WHERE O_ORDERKEY = $okey"
        if { $REFRESH_VERBOSE } {
...
proc do_refresh { host port socket ssl_options user password db scale_factor update_sets trickle_refresh REFRESH_VERBOSE RF_SET oceanbase_db ob_tenant_name} {
    set vsql_handler [ ConnectToVillageSQL $host $port $socket $ssl_options $user $password $db $oceanbase_db $ob_tenant_name]
    set upd_num 1
    for { set set_counter 1 } {$set_counter <= $update_sets } {incr set_counter} {
        ...
        if { $RF_SET eq "RF1" || $RF_SET eq "BOTH" } {
            ...
            mk_order_ref $vsql_handler $upd_num $scale_factor $trickle_refresh $REFRESH_VERBOSE
        ...
        if { $RF_SET eq "RF2" || $RF_SET eq "BOTH" } {
            ...
            del_order_ref $vsql_handler $upd_num $scale_factor $trickle_refresh $REFRESH_VERBOSE
...
proc set_vsql_query { myposition } {
    global sql
...
proc do_tpch { host port socket ssl_options user password db scale_factor RAISEERROR VERBOSE total_querysets myposition vsql_tpch_obcompat ob_tenant_name} {
    global mysqlstatus
    set vsql_handler [ ConnectToVillageSQL $host $port $socket $ssl_options $user $password $db $vsql_tpch_obcompat $ob_tenant_name]
    if { $myposition <= 1 } {
    ...
    set count [ standsql $vsql_handler $countsql $RAISEERROR ]
    if { $count } {
    ...
    for {set it 0} {$it < $total_querysets} {incr it} {
        if {  [ tsv::get application abort ]  } { break }
        catch {set engine [ join [ mysql::sel $vsql_handler "SELECT case when count(NAME) = 8 then 'Heatwave' else 'InnoDB' end as hw_loaded_all FROM performance_schema.rpd_table_id where NAME like '$db.%'" -list ] ]}
        if { $mysqlstatus(code)  } {
            catch {set engine [  join [ mysql::sel $vsql_handler  "select distinct(engine) from information_Schema.tables where table_schema = '$db'" -list] ]}
        }
        if { $mysqlstatus(code)  } {
            set engine ""
        }
        ...
        mysql::autocommit $vsql_handler $autocommit
        for { set q 1 } { $q <= 22 } { incr q } {
            set rowcount 0
            if {  [ tsv::get application abort ]  } { break }
            ...
            if {$qos != 15} {
                ...
                set oput [ standsql $vsql_handler $dssquery($qos) $RAISEERROR ]
                ...
                while {$q15c <= [expr $q15length - 1] } {
                    if { $q15c != 1 } {
                        if {[ catch {mysqlexec $vsql_handler $dssquery($qos,$q15c)} ] } {
                            if { $mysqlstatus(code)  } {
                                if { $RAISEERROR } {
                                    error "Query Error : $mysqlstatus(message)"
                                } else {
                        ...
                        set t0 [clock clicks -millisec]
                        catch { set oput [ mysql::sel $vsql_handler $dssquery($qos,$q15c) ] }
                        if { $mysqlstatus(code)  } {
                            if { $RAISEERROR } {
                                error "Query Error : $mysqlstatus(message)"
                            } else {
...
proc set_query { } {
    ...
    #No Median or Percentile functions in VillageSQL
    ...
    #No HyperLogLog Approx_count_distinct function in VillageSQL replaced with count(distinct)
...
proc do_cloud { host port socket ssl_options user password db RAISEERROR VERBOSE is_oceanbase ob_tenant_name} {
    global mysqlstatus
    set vsql_handler [ ConnectToVillageSQL $host $port $socket $ssl_options $user $password $db $is_oceanbase $ob_tenant_name]
    mysqlexec $vsql_handler "set session group_concat_max_len = 18446744073709551615"
    unset -nocomplain qlist
    set start [ clock seconds ]
    for { set q 1 } { $q <= 13 } { incr q } {
        ...
        set t0 [clock clicks -millisec]
        set oput [ standsql $vsql_handler $query $RAISEERROR ]
        set t1 [clock clicks -millisec]
        set value [expr {double($t1-$t0)/1000}]
        set rowcount [ llength $oput ]
...
proc delete_vsqltpch {} {
    ...
    if {[ tk_messageBox -title "Delete Schema" -icon question -message "Do you want to delete the [ string toupper $vsql_tpch_dbase ] TPROC-H schema\n in host [string toupper $vsql_connector] under user [ string toupper $vsql_tpch_user ]?" -type yesno ] == yes} {
        ...
        set ntimes 1
        ed_edit_clear
        set _ED(packagekeyname) "VillageSQL TPROC-H deletion"
        if { [catch {load_virtual} message]} {
...
proc drop_schema { host port socket ssl_options user password dbase is_oceanbase ob_tenant_name} {
    ...
    set vsql_handler [ ConnectToVillageSQL $host $port $socket $ssl_options $user $password $is_oceanbase $ob_tenant_name]
    ...
    return
        ...
        .ed_mainFrame.mainwin.textFrame.left.text fastinsert end "drop_schema $vsql_host $vsql_port $vsql_socket {$vsql_ssl_options} $vsql_tpch_user [ quotemeta $vsql_tpch_pass ] $vsql_tpch_dbase $vsql_tpch_obcompat $vsql_ob_tenant_name"
    } else { return }
}
...
proc check_vsqltpch {} {
    ...
    if {[ tk_messageBox -title "Check Schema" -icon question -message "Do you want to check the [ string toupper $vsql_tpch_dbase ] TPROC-H schema\n in host [string toupper $vsql_connector] under user [ string toupper $vsql_tpch_user ]?" -type yesno ] == yes} {
        set maxvuser 1
        set suppo 1
        set ntimes 1
        ed_edit_clear
...
proc check_tpch { host port socket ssl_options user password dbase scale_factor is_oceanbase ob_tenant_name } {
   ...
   #Check 1 Database Exists
	...
	puts "Check consistency"
    	set checkconsistency [  mysql::sel $vsql_handler "SELECT * FROM (SELECT o_orderkey, o_totalprice - SUM(truncate(truncate(l_extendedprice * (1 - l_discount),2) * (1 + l_tax),2)) part_res FROM ORDERS, LINEITEM WHERE o_orderkey=l_orderkey GROUP BY o_orderkey, o_totalprice) temp WHERE not part_res=0" -flatlist ]
    ...
    return
        ...
        .ed_mainFrame.mainwin.textFrame.left.text fastinsert end "check_tpch $vsql_host $vsql_port $vsql_socket {$vsql_ssl_options} $vsql_tpch_user [ quotemeta $vsql_tpch_pass ] $vsql_tpch_dbase $vsql_scale_fact $vsql_tpch_obcompat $vsql_ob_tenant_name"
    } else { return }
}

proc tcount_vsql {bm interval masterthread} {
    global tc_threadID vsql_ssl_options
    ...
    set tc_threadID [thread::create {
        ...
        proc ConnectToVillageSQL { MASTER host port socket ssl_options user password } {
            ...
            if [catch {set vsql_handler [eval $login_command]} message ] {
                set connected "false"
            } else {
                set connected "true"
            }
        ...
        proc read_more { MASTER library vsql_host vsql_port vsql_socket vsql_ssl_options vsql_user vsql_pass vsql_tpch_user vsql_tpch_pass interval old tce bm } {
            ...
            while { $timeout eq 0 } {
                ...
                if {[catch {set handler_stat [ list [ mysql::sel $vsql_handler $sqc -list ] ]} message]} {

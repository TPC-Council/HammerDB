...
set install_message "Ready to create a Scale Factor $scale_fact TimesTen HDB TPC-H schema\nin the existing database [string toupper $instance]\n under existing user [ string toupper $tpch_user ]?"
	} else {
set install_message "Ready to create a Scale Factor $scale_fact HDB TPC-H schema in database [string toupper $instance]\n under user [ string toupper $tpch_user ] in tablespace [ string toupper $tpch_def_tab]?"
	}
...
set ntimes 1
ed_edit_clear
set _ED(packagekeyname) "Oracle HDB TPC-H creation"
...
set _ED(packagekeyname) "Oracle HDB TPC-H"
.ed_mainFrame.mainwin.textFrame.left.text fastinsert end "#!/usr/local/bin/tclsh8.6
#EDITABLE OPTIONS##################################################
